...
def test_max_matches_partial(runtime, authz):
    result = handle_tool(
        runtime,
        "drive_grep",
...
def test_ls_pagination_not_complete_when_more_remain(runtime, authz):
    result = handle_tool(
        runtime,
        "drive_ls",
...
def test_walk_rate_limit_is_partial_not_error_envelope(runtime, fake_drive, authz):
...
def test_single_file_429_is_error_envelope_rate_limited(runtime, fake_drive, authz):
