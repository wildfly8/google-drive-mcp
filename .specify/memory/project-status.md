...
| Current phase | Converged (2026-09-13): MCP OAuth 2.1 caller auth (AC-FR-012) replaces in-process shared-secret Bearer |
| Next command | Review. Deploy is required before Cloud Run `/mcp` rejects `MCP_AUTH_TOKEN` as Bearer. Raise a PR only if the user asks. |
