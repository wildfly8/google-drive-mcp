...
def _note_drive_first_use(runtime: Runtime, authorization: str | None) -> None:
    claims = verify_access_claims(extract_bearer(authorization), runtime.settings)
    if claims is None:
        return
    connect_id = claims.get("cid")
    if not isinstance(connect_id, str) or not connect_id:
        return
    runtime.telemetry.record_drive_first_use(
        connect_id,
        host_family_from_client_id(str(claims.get("client_id") or "")),
    )
