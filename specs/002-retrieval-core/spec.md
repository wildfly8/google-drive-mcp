...
- Q: When `folder_id` is omitted and the deployment sets `DRIVE_ALLOWED_FOLDER_ID`, does `drive_ls` still list My Drive `root`? → A: No. Access Control (AC-FR-021) rewrites an omitted folder to that allow-listed folder before retrieval runs — narrow-only, never wider than Google. `drive_ls` then lists that folder’s immediate children. Without an allow-list, omitted-folder `ls` still projects `default_whole_grant` onto My Drive `root` children. Owned by Access Control; consumed here.
...
- Upstream Drive failures MUST be `DRIVE_API_ERROR` (or equivalent upstream-failure category) rather than a generic error (Article XI). Authentication and authorization errors are raised by Access Control, not this context. `drive_read` / `drive_ls` / `drive_find` emit `AUTHORIZATION_ERROR` in v1 only when Access Control’s deployment allow-list (`DRIVE_ALLOWED_FOLDER_ID`) rejects a Google-granted named resource; Google misses remain `FILE_NOT_FOUND`.
...
- **FR-001**: MUST enumerate the immediate children of a given `folder_id`, returning `id, name, mime_type, is_folder, modified_time, source_url` per child. `source_url` MUST be the non-dereferenceable locator `drive:{id}` (not Drive `webViewLink` and not any `http`/`https` URL). When `folder_id` is omitted, MUST enumerate the immediate children of My Drive `root` (a projection of `default_whole_grant`, not a second source of truth), **unless** Access Control has already rewritten the omitted folder to deployment `DRIVE_ALLOWED_FOLDER_ID` (AC-FR-021, narrow-only). In that case MUST list that folder’s immediate children.
...
- **RetrievalScope**: The bounded set of Drive resources an operation may touch. Field `default_whole_grant` is true when the call names no `folder_id` or `file_ids` (everything the deployment’s Google grant already allows). Access Control MAY rewrite an omitted folder to `DRIVE_ALLOWED_FOLDER_ID` before this context runs (narrow-only, AC-FR-021). A named folder or file list narrows that call only. Enforced by Access Control via `is_within_scope`. `drive_ls` with true `default_whole_grant` (no allow-list rewrite) lists immediate children of My Drive `root` only.
