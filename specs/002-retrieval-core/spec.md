...
- Unsupported or non-exportable content: **single-id** read or grep → classified error (`UNSUPPORTED_MIME_TYPE` / `FILE_NOT_EXPORTABLE`), not empty success. **Folder or whole-grant grep** with mixed types → skip unsupported, search the rest, `PARTIAL`; if every target is unsupported → classified error. v1 required types are Google Docs, Sheets, and Slides; other Drive files are supported only when they yield usable text.
