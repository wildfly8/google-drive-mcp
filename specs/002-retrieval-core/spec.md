...
3. **Given** sustained rate-limiting that cuts a search short, **When** the operation returns, **Then** that is reported as a completeness event (`PARTIAL` or `RATE_LIMITED`), not swallowed by a silent retry that claims completeness.
...
- **FR-050**: Every content-derived result MUST include, at minimum: `file_id, file_name, mime_type, modified_time, source_url, matched_text/location, retrieved_at`. A result missing `file_id` is invalid (Article VIII).
...
- **FR-104**: Every operation SHOULD have bounded consumption (`max_files, max_bytes, max_matches, max_execution_time, max_context_lines, max_export_size`) and MUST fail as `PARTIAL` with a reason rather than silently overflowing. Exact numeric defaults are plan-level.
...
| I7 | Temporary content does not survive beyond its operation | Article III |
| I8 | Deterministic grep for identical bytes and parameters, within a request | Article IX |
| I9 | A new read or grep never relies on a stale persistent copy | Articles I, IX |
| I10 | Discovery results are never represented as verified evidence | Article VIII |
...
- **SC-005**: When a search or listing hits a configured limit while more coverage was possible, 100% of those outcomes are reported as partial (or rate-limited completeness), never as empty or complete.
